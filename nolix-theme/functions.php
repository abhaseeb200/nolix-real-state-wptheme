...
{
    if (!isset($_POST['nolix_team_details_nonce']))
        return;
    if (!wp_verify_nonce($_POST['nolix_team_details_nonce'], 'nolix_save_team_details'))
        return;
...
/* Removed nolix_add_property_meta_boxes, nolix_property_details_callback, nolix_save_property_details since the property details are managed by the API sync */

// Add Custom Columns for Properties
function nolix_set_property_columns($columns)
{
    $newColumns = array();
    $newColumns['cb'] = $columns['cb'];
    $newColumns['title'] = $columns['title'];
    $newColumns['listingType'] = __('Listing Type', 'nolix');
    $newColumns['status'] = __('Status', 'nolix');
    $newColumns['date'] = $columns['date'];
    return $newColumns;
}
add_filter('manage_property_posts_columns', 'nolix_set_property_columns');

function nolix_show_property_custom_columns($column, $post_id)
{
    switch ($column) {
        case 'listingType':
            echo esc_html(get_post_meta($post_id, 'listingType', true));
            break;
        case 'status':
...
add_action('manage_property_posts_custom_column', 'nolix_show_property_custom_columns', 10, 2);

function nolix_sortable_property_columns($columns)
{
    $columns['listingType'] = 'listingType';
    $columns['status'] = 'status';
    return $columns;
}
add_filter('manage_edit-property_sortable_columns', 'nolix_sortable_property_columns');
...
{
    if (!isset($_POST['nolix_testimonial_details_nonce']))
        return;
    if (!wp_verify_nonce($_POST['nolix_testimonial_details_nonce'], 'nolix_save_testimonial_details'))
        return;
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)
        return;
    if (!current_user_can('edit_post', $post_id))
...
// Include API Properties Sync
require_once get_template_directory() . '/inc/api-properties-sync.php';
